...
const ProfilePage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (firstname === UserData.fullname.firstname && lastname === UserData.fullname.lastname && address === UserData.address) {
            return toast.error("Cannot update same data")
        }

        const token = localStorage.getItem('token')
        const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/users/update-profile`, {
    ...
    const VerifyBtnHandler = async () => {
        try {
            ...
            const response = await axios.post(
                `${import.meta.env.VITE_BASE_URL}/users/verify-email`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${token}`
    ...
    return (
        <>
            ...
            {openVerify && <VerifyOtpModal userType={'users'} onVerify={onVerifySuccess} onClose={() => setOpenVerify(false)} />}
